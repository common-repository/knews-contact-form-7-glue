...
class knews_cf7 {
   ...
   function __construct() {
		...
		//add_filter( 'wpcf7_validate_knewssubscription', array($this, 'knewssubscription_validation'), 10, 2 );
		add_filter( 'wpcf7_validate_knewssubscription*', array($this, 'knewssubscription_validation'), 10, 2 );
	}
	...
	function add_shortcode() {
		if( function_exists('wpcf7_add_form_tag') ) {
			wpcf7_add_form_tag( array( 'knewssubscription', 'knewssubscription*' ),
	...
	function shortcode_handler( $tag ) {
		...
		if ( ! empty ( $checkbox_values ) ) {
			...
			if (current_user_can('edit_posts')) return '[Knews CF7 Glue]: No mailing list selected, or it is closed to new subscribers';
			return '';
		}
		...
		$html = sprintf(
			'<span class="wpcf7-form-control-wrap %1$s"><input type="checkbox" %2$s />&nbsp;<label for="%3$s">%4$s</label>&nbsp;%5$s</span>',
			$tag->name, $atts, $id, $label, $validation_error );
			

		//$lang = $Knews_plugin->pageLang();
		$langs = $Knews_plugin->getLangs(true);
		foreach ($langs as $lang) {
			if ($lang['active'] == 1 && count($lists) != 0) {
	...
	function knewssubscription_validation( $result, $tag ) {
		...
		if ($value==0) $result->invalidate( $tag, wpcf7_get_message( 'invalid_required' ) );
	...
	function tag_pane( $contact_form ) {
		global $Knews_plugin;
		$old_cf7 = true;
		if (defined('WPCF7_VERSION') && version_compare(WPCF7_VERSION, '4.2') >= 0) $old_cf7 = false;
		?>
		<div id="wpcf7-tg-pane-knewssubscription" <?php echo ($old_cf7 ? ' class="hidden"' : ''); ?>>
		<form action="">
			<?php if (!$old_cf7) echo '<div class="control-box">'; ?>
			<table>
				<tr>
					<td>
			...
			<table>
				<tr>
					...
					<td>
						...
						<textarea name="values" placeholder="<?php __( 'Subscribe me to the newsletter', 'knews_cf7' ); ?>"></textarea>
					</td>
				</tr>
				<tr>
					<td colspan="2">
				...
				<tr>
						<?php
						if ($old_cf7) echo '<td colspan="2">';
						$nfield=0;
						
						$extra_fields = $Knews_plugin->get_extra_fields();
						foreach ($extra_fields as $f) {
							if (!$old_cf7 && $nfield != 0 && $nfield%2==0) echo '</tr><tr>';
							$nfield++;
							if (!$old_cf7) echo '<td>';
...
?>
			...
			<div class="insert-box">
				<input type="text" name="knewssubscription" class="tag code" readonly onfocus="this.select()" style="float:none;" />

				<div class="submitbox">
					<div class="tg-mail-tag" style="position:absolute; margin-right:100px;"><?php echo esc_html( __( "Use the value input through this field in a mail field into the field on the Mail tab.", 'knews_cf7' ) ); ?><br /><input type="text" class="mail-tag" readonly onfocus="this.select()" /></div>
